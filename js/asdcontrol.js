...
class AzimuthControl2D extends BaseControl{constructor(e){super(),this.canvas=document.getElementById(e),this.canvas.width=this.canvas.getBoundingClientRect().width,this.canvas.height=this.canvas.getBoundingClientRect().height,this.ctx=this.canvas.getContext("2d"),this.radius=this.canvas.width/2,this.center={x:this.radius,y:this.radius},this.state={angle:0,thrust:0},this.handlePosition={x:this.center.x,y:this.center.y},this.isDragging=!1,this.isReturningToCenter=!1,this.lastClickTime=0,this._initListeners(),this.animate()}
_initListeners(){this.canvas.addEventListener("pointerdown",this._handlePointerDown.bind(this)),window.addEventListener("pointermove",this._handlePointerMove.bind(this)),window.addEventListener("pointerup",this._handlePointerUp.bind(this))}
_handlePointerDown(e){this.isReturningToCenter=!1;const t=Date.now(),s=t-this.lastClickTime;s<300?(this.fire("doubleclick"),this.isDragging=!1,this.lastClickTime=0):((this.lastClickTime=t,this.isDragging=!0,this._updateControl(e)))}
_handlePointerMove(e){this.isDragging&&!this.isReturningToCenter&&this._updateControl(e)}
_handlePointerUp(e){this.isDragging&&(this.isDragging=!1)}
